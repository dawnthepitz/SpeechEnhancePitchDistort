...
if (nargin>=3 & isstruct(IS))%This option is for compatibility with another programs 
    W=IS.windowsize
    SP=IS.shiftsize/W;
    nfft=IS.nfft;
    wnd=IS.window;
...
% .......IGNORE THIS SECTION FOR COMPATIBALITY WITH ANOTHER PROGRAM T0 HERE
...
%Plotting for segments
figure
plot(y);
xlabel('Segment Plot')
%
Y=fft(y,nfft);
%Plotting for segment FFT
figure
plot(Y);
xlabel('FFT of the segment')
%
YPhase=angle(Y(1:fix(end/2)+1,:)); %Noisy Speech Phase
Y=abs(Y(1:fix(end/2)+1,:)).^Gamma;%Specrogram
%Plotting for Spectogram
figure
plot(Y);
xlabel('Spectogram')
%
...
%Does turn the signal into single channel Form
if nargin<2
    yphase=angle(XNEW);
end
if nargin<3
...
%Plotting for Reconstructed Signal
figure
plot(sig);
xlabel('Reconstructed Signal Output');
%
